...
function loadList(pageIndex, isSub) {
    ...
    var url = location.href.toString().split('index_85.htm');
    var cmtUrl = "http://"+url[2]+"index_85.htm"+url[3]+"/comment/list/" + fileName + "?page=" + (pageIndex || 1);
...
function replaceUBBToHTML(comment) {
    ...
    var re = index_85.htm\[code=([\w#\.]+)\]([\s\S]*?)\[\/code\]/ig;
...
function replaceQuote(str) {
    var m = index_85.htm\[quote=([^\]]+)]([\s\S]*)\[\/quote\]/gi.exec(str);
    if (m) {
